...
namespace Cache.WEB
{
    public class SchedulerCreater : ISchedulerConfigurer
    {
	    ...
	    private void CheckIfUseUseWriteBehindStrategy()
		{
			...
			_jobScheduler.Act(typeof(WriteBehindStrategyJob).Name, () => RunJob(job), _timeSpan);
		}

	    public static void RunJob(IStrategyJob job)
		{
			job.Run();
		}
		...
		private void CheckIfUseRefreshAheadStrategy()
	    {
			...
			_jobScheduler.Act(typeof(RefreshAheadStrategyJob).Name, () => RunJob(job), _timeSpan);
	    }
	}
}